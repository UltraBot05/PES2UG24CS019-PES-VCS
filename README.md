...
| File                 | Role                                 | Your Task (Done)                                          |
| -------------------- | ------------------------------------ | --------------------------------------------------------- |
| `pes.h`            | Core data structures and constants   | Do not modify                                             |
| `object.c`         | Content-addressable object store     | Implemented `object_write`, `object_read`             |
| `tree.h`           | Tree object interface                | Do not modify                                             |
| `tree.c`           | Tree serialization and construction  | Implemented `tree_from_index`                           |
| `index.h`          | Staging area interface               | Do not modify                                             |
| `index.c`          | Staging area (text-based index file) | Implemented `index_load`, `index_save`, `index_add` |
| `commit.h`         | Commit object interface              | Do not modify                                             |
| `commit.c`         | Commit creation and history          | Implemented `commit_create`                             |
...
3. Read current commit hash from HEAD (through current ref if symbolic).
4. Parse both commit objects and load their root trees.
5. Build file maps for current-tree vs target-tree (path -> mode, hash).
6. Perform safety checks for uncommitted changes before touching files.
7. Update working directory to match target tree:
   - Delete tracked files present in current tree but absent in target tree.
   - Overwrite files whose blob hash differs.
   - Create new files/directories from target tree blobs.
   - Apply executable bit from mode.
8. Rewrite index to exactly match target tree snapshot.
9. Update `.pes/HEAD` to `ref: refs/heads/<branch>`.

Complexity comes from correctness and safety: conflict detection, recursive tree materialization, delete-vs-overwrite order, mode updates, and atomicity if checkout fails midway.

**Q5.2:** When switching branches, the working directory must be updated to match the target branch's tree. If the user has uncommitted changes to a tracked file, and that file differs between branches, checkout must refuse. Describe how you would detect this "dirty working directory" conflict using only the index and the object store.

Use index as the expected baseline and object store as truth for staged content:

1. For each indexed path, `stat()` working file.
2. If file missing, mark dirty.
3. If mtime/size differs from index entry, re-hash file and compare with indexed blob hash.
4. If hash differs, file has unstaged changes.
5. Build diff between current-branch tree and target-branch tree.
6. If a dirty file is in the branch-diff set (modified/deleted/added incompatibly), refuse checkout.

So conflict condition is:
dirty(path) AND branch_changes(path)

This prevents clobbering local edits.

**Q5.3:** "Detached HEAD" means HEAD contains a commit hash directly instead of a branch reference. What happens if you make commits in this state? How could a user recover those commits?

In detached HEAD, new commits are created normally, but no branch ref is moved. HEAD points directly to a commit hash, so the new commits can become unreachable when HEAD moves elsewhere.

Recovery options:

1. While still detached, create a branch at current commit (`refs/heads/<name>` -> current hash).
2. If already moved away, recover via reflog-like history (or manual saved hash), then create a branch at that commit.

Creating a branch reattaches that line of history and prevents GC from collecting it.
...
1. Roots = all branch heads in `.pes/refs/heads/*` (and tags if implemented).
2. Traverse each root commit graph (follow parent links).
3. For each visited commit, mark its tree hash reachable.
4. Recursively parse each tree; mark child trees and blob hashes reachable.
5. After marking, iterate all files under `.pes/objects/*/*`.
6. Delete any object hash not in reachable set.

Data structure: hash set of object IDs (O(1) average insert/lookup).

Scale estimate for 100,000 commits and 50 branches:

1. Commit visits: at most ~100,000 unique commits (branches usually overlap).
2. Tree/blob visits depend on churn; can be several times commits.
3. Total visited objects is typically in the low millions for medium projects, but upper bound is repository-size dependent.
...
2. Before branch ref is updated, GC scans refs and does not see those new objects as reachable.
3. GC deletes them.
4. Commit process then writes commit object and updates ref to point to commit whose tree/blob was deleted.
5. Repository now references missing objects (corruption).

How Git avoids this:

1. Uses lock files and reference transaction semantics around ref updates.
2. Uses temporary object retention and grace periods (reflog-based reachability windows).
3. Runs GC with coordination so newly created objects are not reclaimed prematurely.
